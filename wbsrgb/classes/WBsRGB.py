...
class WBsRGB:
    def __init__(self, gamut_mapping=2, upgraded=0):
        ...
        if upgraded == 1:
            self.features = np.load('wbsrgb/models/features+.npy') # training encoded features
            self.mappingFuncs = np.load('wbsrgb/models/mappingFuncs+.npy') # mapping correction functions
            self.encoderWeights = np.load('wbsrgb/models/encoderWeights+.npy') # weight matrix for histogram encoding
            self.encoderBias = np.load('wbsrgb/models/encoderBias+.npy') # bias vector for histogram encoding
